...
import (
	...
	"github.com/noradomi/coffeeshop-clone/internal/counter/infras/postgresql"
	"github.com/noradomi/coffeeshop-clone/internal/counter/usecases/orders"
	shared "github.com/noradomi/coffeeshop-clone/internal/pkg/shared_kernel"
	"github.com/noradomi/coffeeshop-clone/pkg/postgres"
...
func (d *orderRepo) GetAll(ctx context.Context) ([]*domain.Order, error) {
	...
	queries := postgresql.New(d.pg.GetDB())

	results, err := queries.GetAll(ctx)
	if err != nil {
		return nil, errors.Wrap(err, "queries.GetAll")
	}
...
func (d *orderRepo) GetByID(ctx context.Context, id uuid.UUID) (*domain.Order, error) {
	...
	results, err := queries.GetByID(ctx, id)
	if err != nil {
		return nil, errors.Wrap(err, "queries.GetByID")
	}
